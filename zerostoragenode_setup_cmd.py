...
class ZeroStorageNode():
    ...
    def install_openssh_server(self):
        try:
            check_installed = subprocess.run(["cmd", "/c", "powershell.exe Get-WindowsCapability -Online | ? Name -like 'OpenSSH.Server*'"], capture_output=True, text=True, check=False)

            if "Installed" not in check_installed.stdout:
                install_command = "Add-WindowsCapability -Online -Name OpenSSH.Server~~~~0.0.1.0"
                subprocess.run(["cmd", "/c", f"powershell.exe {install_command}"], check=True)
    ...
    def start_ssh_service(self):
        try:
            check_service = subprocess.run(["cmd", "/c", "powershell.exe Get-Service sshd"], capture_output=True, text=True, check=False)
            if "sshd" in check_service.stdout:
                subprocess.run(["cmd", "/c", "powershell.exe Start-Service sshd"], check=True)
                self.logger.info("SSH service started.")

                subprocess.run(["cmd", "/c", "powershell.exe Set-Service -Name sshd -StartupType Automatic"], check=True)
    ...
    def append_pub_key(self):
        ...
        subprocess.run(["cmd", "/c", f'echo {pub_key} >> "%USERPROFILE%\.ssh\authorized_keys"'], capture_output=True, text=True, check=False)

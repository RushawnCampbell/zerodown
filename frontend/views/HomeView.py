...
from frontend.components.JobRestore import JobRestore
import datetime
import textwrap
...
class HomeView(gui.CTkFrame):
    def __init__(self, master):
        ...
        self.grid_rowconfigure(1, weight=0) # For the recent jobs frame
        self.grid_rowconfigure(2, weight=0) # For the storage info label
        self.grid_rowconfigure(3, weight=0) # For the storage info frame
        ...
        self._populate_recent_frame(row_limit=3) # Show fewer initial items
        ...
        self._populate_storage_frame(row_limit=3)
    ...
    def _populate_recent_frame(self, row_limit=None):
    ...
    def _populate_storage_frame(self, row_limit=None):
        ...
        for i, data in enumerate(sample_storage_data):
            if row_limit is not None and i >= row_limit:
                break
            row_num = i + 1
    ...
    def _restore_job(self, job_name):
        ...
        if response:
            JobRestore(self, f"Restoring {job_name}")

            ...
            if namepart == fetched_user.username.lower():
                try:
                    if object_type == "endpoints":
                        ...
                        print("JOBS ARE", fetched_scheduled_jobs)
                        scheduled_jobs = []
                        for scheduled_job in fetched_scheduled_jobs:
                            print("IIDDDD", scheduled_job.existing_job.last_backup)
    ...
    def schedule():
        ...
        try:
            ...
            if sch_frequency == "Weekly":
                scheduled_job = ScheduledJob(job_id=job_id, frequency=sch_frequency, sch_datetime = sch_datetime, id=sch_id, endpoint_id= endpoint_id, sch_day = sch_day )
            else:
                scheduled_job = ScheduledJob(job_id=job_id, frequency=sch_frequency, sch_datetime = sch_datetime, id=sch_id, endpoint_id= endpoint_id)
